...
export default function SDCEligibility() {
  return (
    ...
    >
      ...
      <p className="cs-p reveal">My EM dropped the ADO item in my queue to take a look. What I found was that the endpoint wasn't broken at all. The spike tracked almost exactly with increased traffic. A lot of those guests simply weren't eligible for SDC, and the BFF was returning a 404 for them every time.</p>
      ...
      <div className="cs-callout reveal">
        ...
        <p>Importantly, this did not affect the guest experience. When the 404 came back, the frontend simply didn't show the Same Day Change button. Guests who weren't eligible never saw a broken page or an error. The impact was entirely behind the scenes: misleading monitoring signals and a false picture of endpoint health.</p>
      </div>

      <h2 className="cs-h2 reveal">How I <span className="it">traced it.</span></h2>
      <p className="cs-p reveal">The QM team shared session recordings that made the 404s visible. I took those sessions into QA and reproduced the issue myself: hit the endpoint for an ineligible PNR, got a 404 back. That was enough to know something was off.</p>
      ...
      <p className="cs-p reveal">This feature had been gated behind an Optimizely toggle since it was built about eight months ago, so it's possible the behavior had been there the whole time and just hadn't been visible at scale until traffic picked up.</p>
      ...
      <h2 className="cs-h2 reveal">Reframing it for <span className="it">stakeholders.</span></h2>
      <p className="cs-p reveal">The framing mattered here. The initial concern from QM was that the endpoint was broken and generating errors. That framing would have made this an incident. What was actually happening was a logic bug that misclassified a normal outcome as an HTTP error. The endpoint was functioning, just returning the wrong status code for a valid business case.</p>
      <p className="cs-p reveal">I documented my findings directly in the ADO item, including the root cause, the expected behavior, and the proposed fix. The fix itself is simple: when the downstream API returns a successful response with eligible: false, return a 200 with that payload instead of throwing a 404. No ambiguity in the outcome, just wrong response code.</p>

      <h2 className="cs-h2 reveal">Ownership without <span className="it">history.</span></h2>
      <p className="cs-p reveal">The engineer who originally built this feature is no longer on the team. That could have been a reason to deprioritize it or treat it as someone else's problem. It wasn't. It's our product, and when something lands in my queue I treat it as mine regardless of who wrote the original code or how well I know that part of the codebase.</p>
      <p className="cs-p reveal">I didn't have deep context on this feature going in. I learned what I needed to: how the endpoint was structured, what the gatherer was doing, what the downstream API actually returned. That was enough to identify the root cause and write a clear fix proposal. With my workload already full, I routed the implementation to one of our newer engineers. The ADO item had everything they needed — root cause, reproduction steps, proposed fix — so the handoff required no back-and-forth.</p>
      ...
      <ol className="cs-decisions reveal">
        ...
        <li><strong>Reframed the narrative early.</strong> Communicating "logic bug, not broken endpoint" to the PM and EM before the fix was scoped prevented unnecessary urgency and kept the scope accurate.</li>
